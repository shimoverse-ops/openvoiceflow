...
**The website.** The download site (`openvoiceflow.com`) separately uses **Vercel Analytics** and **Vercel Speed Insights** for anonymous page views, aggregated visitor/referrer/geography trends, and performance — no advertising cookies, no cross-site profile. It also records anonymous counts of which links and buttons get clicked (downloads, navigation, calls to action, footer links, copying a checksum, and opening a release note or FAQ answer), so we can tell which pages and which answers are actually useful. These are counts of clicks on our own pages, tied to no identifier and to no visitor. This is independent of the in-app usage summary above. Vercel's analytics privacy notice: <https://vercel.com/docs/analytics/privacy-policy>.
